...
function unbackedClaim(text, material) {
  ...
  // ٣) حكمٌ من عندِه: وضعٌ أو نفيُ أصلٍ أو تضعيفٌ لم تنطقْ به المادّة — وهذا
  //    أخطرُ ما يكون في الحديث، فالدرجاتُ منقولةٌ لا مُنشأة، وغيابُ اللفظِ عن
  //    الموسوعةِ حدُّ تغطيةٍ لا حكمٌ بالوضع. (النصُّ مُطبَّعٌ لأنَّ \b لا تعملُ
  //    مع العربيّةِ في JS، فنستعملُ الفراغاتَ حدودًا للكلمات.)
  const t = ` ${guardNorm(text)} `, h = ` ${hay} `;
  const VERDICTS = [
    [/ (?:موضوع|مكذوب|مختلق) /, "موضوع/مكذوب"],
    [/ لا اصل له /, "لا أصل له"],
    [/ لا يصح (?:عن|من|هذا|اللفظ) /, "لا يصح"],
    [/ (?:حديث )?باطل /, "باطل"],
  ];
  for (const [re, label] of VERDICTS) {
    if (!re.test(t)) continue;
    if (re.test(h)) continue;                       // the material itself records it
    return `أطلقتَ حكمًا («${label}») لم تنطقْ به المادّة. الدرجاتُ منقولةٌ كما `
      + `دُوِّنت، وغيابُ اللفظِ عن الموسوعةِ حدُّ تغطيةٍ لا حكمٌ بالوضع. `
      + `أعِدْ الجوابَ بلا هذا الحكم.`;
  }
  return null;
}
