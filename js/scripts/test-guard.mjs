...
const CASES = [
  ...
  ["نقلُ حكمٍ منصوصٍ في المادّة", "الحكمُ المسجَّلُ: صحيح، وأقوى درجاتِ طرقِه صحيح.", null],
  ...
  ["حكمٌ بالوضعِ لم تنطقْ به المادّة", "وهذا الحديثُ موضوعٌ لا يصحُّ الاستدلالُ به.", "verdict"],
  ["نفيُ الأصلِ من عندِه", "هذا اللفظُ لا أصلَ له في كتب السنّة.", "verdict"],
  ["تضعيفٌ من عندِه", "وهو لا يصحُّ عن النبيِّ ﷺ.", "verdict"],
];

let pass = 0, fail = 0;
for (const [name, text, expect] of CASES) {
  const got = unbackedClaim(text, MATERIAL);
  const WANT = { quote: "اقتبستَ", number: "رقمًا", verdict: "حكمًا" };
  const ok = expect === null ? got === null : got !== null && got.includes(WANT[expect]);
